        ...
        if not user:
            ...
            for i, col in enumerate(cursor.description):
                ...
                if key in ['theft_last_use', 'last_login_date', 'last_transfer_reset', 'last_special_coin_payout',
                                           ...
                                           'last_mine_reset', 'last_fish_reset', 'last_hunt_reset', 'luck_potion_end_time', 'last_restaurant_payout'] and value is not None:
                    try:
                        user_dict[key] = datetime.datetime.fromisoformat(value)
                    except ValueError:
                        user_dict[key] = None # Handle invalid date strings
                elif key in ['restaurants', 'restaurants_max_count', 'activated_promos']:
    ...
    if not user_doc_exists_before_get:
        if referrer_id and referrer_id != user_id:
            ...
            if referrer_user:
                ...
                reward_amount = 10000 + (current_invites * 1000) # 10k for the first, 11k for second, etc.
                
                referrer_user['invites_count'] = current_invites + 1
                ...
                try:
                    await bot.send_message(referrer_id, f"Пользователь <code>{user_id}</code> (<b>{user_name_escaped}</b>) зарегистрировался по вашей ссылке! Вы получили <b>{reward_amount:,}</b> обычных монет.", parse_mode="HTML")
                except Exception as e: print(f"Сообщение рефереру {referrer_id} не отправлено: {e}")
...
async def command_hunt_handler(message: Message):
    ...
    effect = await get_user_effect(message.from_user.id)
    if effect:
        if effect["type"] == "curse_weakness":
            money_earned //= 2
        elif effect["type"] == "curse_vision":
            money_earned *= 3
            await clear_user_effect(message.from_user.id)  # действует один раз
        elif effect["type"] == "curse_vampire":
            # выбираем случайного другого игрока
            async with aiosqlite.connect(DB_PATH) as db:
                cursor = await db.execute('SELECT _id FROM users WHERE _id != ?', (message.from_user.id,))
                all_users = [row[0] for row in await cursor.fetchall()]
            if all_users:
                victim = random.choice(all_users)
                vamp_part = int(money_earned * 0.1)
                money_earned -= vamp_part
                victim_data = await get_user_data(victim)
                victim_data["balance_halloween"] = victim_data.get("balance_halloween", 0) + vamp_part
                await update_user_data(victim, victim_data)
...
@dp.message(Command("my_curses"))
@dp.message(F.text.lower().in_(("мое проклятие", "проклятия", "curse", "проклятья")))
async def command_my_curses(message: Message):
    """Показывает активные проклятия или благословения игрока."""
    user_id = message.from_user.id
    effect = await get_user_effect(user_id)

    if not effect:
        await message.answer("✨ На вас нет активных проклятий или эффектов. Вы чисты и свободны!", parse_mode="HTML")
        return

    # определяем оставшееся время
    now = datetime.datetime.now()
    remaining = effect["end_time"] - now
    if remaining.total_seconds() < 0:
        await clear_user_effect(user_id)
        await message.answer("✨ Ваши проклятия рассеялись.", parse_mode="HTML")
        return

    mins = int(remaining.total_seconds() // 60)
    secs = int(remaining.total_seconds() % 60)

    # красивые описания эффектов
    effect_names = {
    ...
    effect_name = effect_names.get(effect["type"], f"Неизвестный эффект: {effect['type']}")
    await message.answer(
        f"{effect_name}\n\n⏳ Осталось времени: <b>{mins}м {secs}с</b>",
        parse_mode="HTML"
    )
...
async def command_profile_handler(message: Message):
    ...
    text = (
        ...
        f"🎃 Хэллоуинские монеты: {format_num(user.get('balance_halloween', 0))}\n"
        ...
        #f"🏦 Последний грабёж казны PRO: {format_date_or_none(user.get('last_treasury_rob_pro'))}\n"
        ...
        f" └ Хэллоуин: {halloween_cases}"
    )
    # Отправляем без parse_mode, чтобы текст отображался как есть
...
async def command_balance_handler(message: Message):
    ...
    await message.answer(
        ...
        f"🎃 Хэллоуинские монеты: <b>{user.get('balance_halloween', 0):,}</b>",
        parse_mode="HTML"
    )
...
async def command_exchange_handler(message: Message, command: CommandObject = None):
    ...
    currency_map = {
        ...
        "галактические": "balance_galactic", "галактические_монеты": "balance_galactic", "хэллоуинские": "balance_halloween", "хэллоуинские_монеты": "balance_halloween",
    "halloween": "balance_halloween",
    }
    ...
    if rate_key == ("balance_normal", "balance_halloween"):
        await message.answer("🎃 Обмен обычных монет на хэллоуинские недоступен.", parse_mode="HTML")
        return
...
@dp.message(Command("top_halloween"))
@dp.message(F.text.lower().in_(("топ хэллоуин", "топ хэллоуинских", "топ хэллоуинских монет", "топ хэллоуин", "топ 🎃")))
async def command_top_halloween(message: Message):
    """
    🎃 Топ игроков по количеству хэллоуинских монет.
    Показывает 10 лучших игроков.
    """
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "SELECT _id, balance_halloween FROM users WHERE balance_halloween > 0 ORDER BY balance_halloween DESC LIMIT 10"
        )
        top_players = await cursor.fetchall()

    if not top_players:
        await message.answer("🎃 Пока никто не заработал хэллоуинских монет!")
        return

    text = "🏆 <b>Топ 10 игроков по 🎃 хэллоуинским монетам</b>:\n\n"

    for position, (user_id, coins) in enumerate(top_players, start=1):
        try:
        ...
        medal = "1." if position == 1 else "2." if position == 2 else "3." if position == 3 else f"{position}."
...
@dp.message(Command('halloween_case'))
@dp.message(F.text.lower().in_(("открыть хэллоуинский кейс", "🎃 кейс", "открыть кейс хэллоуин")))
async def command_halloween_case_handler(message: Message):
    user_id = message.from_user.id
    user = await get_user_data(user_id)

    # Проверяем наличие хэллоуинских кейсов
    halloween_cases = await get_user_case_count(user_id, "halloween")
    if halloween_cases <= 0:
        await message.answer("🎃 У вас нет хэллоуинских кейсов!")
        return

    # Списываем один кейс
    removed = await remove_case_from_user(user_id, "halloween", 1)
    if not removed:
        await message.answer("Ошибка: не удалось списать кейс. Попробуйте ещё раз.")
        return

    # --- 🎁 Таблица наград и шансы выпадения ---
    rewards = [
        # (Название награды, Поле в БД, Кол-во (функция), Шанс)
        ("💰 Обычные монеты", "balance_normal", lambda: random.randint(50_000, 150_000), 0.40),
        ("✨ Особые монеты", "balance_special", lambda: random.randint(1, 5), 0.15),
        ("🌌 Галактические монеты", "balance_galactic", lambda: 1, 0.10),
        ("🎃 Хэллоуинские монеты", "balance_halloween", lambda: random.randint(1, 25), 0.25),
        ("💼 Денежный кейс", "case_money", lambda: 1, 0.07),
        ("👑 Мифический кейс", "case_mythical", lambda: 1, 0.03),
    ]

    # Выбираем награду по весам (шансам)
    reward_name, reward_field, reward_func, _ = random.choices(
        population=rewards,
        weights=[r[3] for r in rewards],
        k=1
    )[0]

    reward_amount = reward_func()

    # Начисляем награду
    if reward_field.startswith("case_"):
        await add_case_to_user(user_id, reward_field.split("_", 1)[1], reward_amount)
    else:
        user[reward_field] = user.get(reward_field, 0) + reward_amount
        await update_user_data(user_id, user)

    await message.answer(
        f"🎃 Вы открыли <b>Хэллоуинский кейс</b>!\n"
        f"Вы получили: <b>{reward_amount:,}</b> {reward_name}!",
        parse_mode="HTML"
    )


@dp.message(Command('buy_halloween_case'))
@dp.message(F.text.lower().in_(("купить хэллоуинский кейс", "🎃 купить кейс", "купить кейс хэллоуин")))
async def command_buy_halloween_case_handler(message: Message):
    user_id = message.from_user.id
    user = await get_user_data(user_id)

    cost = 25  # 🎃 цена кейса
    if user.get('balance_halloween', 0) < cost:
        await message.answer(
            f"🎃 Недостаточно хэллоуинских монет!\n"
            f"Нужно: <b>{cost}</b>, у вас: <b>{user.get('balance_halloween', 0)}</b>.",
            parse_mode="HTML"
        )
        return

    # списываем цену и добавляем кейс
    user['balance_halloween'] -= cost
    await update_user_data(user_id, user)
    await add_case_to_user(user_id, "halloween", 1)

    await message.answer(
        f"✅ Вы купили <b>Хэллоуинский кейс</b> за <b>{cost}</b> 🎃 монет!\n"
        f"Напишите «открыть хэллоуинский кейс», чтобы открыть его.",
        parse_mode="HTML"
    )
...
async def command_upgrade_pickaxe_handler(message: Message):
    ...
    if user.get('balance_normal', 0) >= cost:
        ...
        await update_user_data(message.from_user.id, user)
        await message.answer(f"⛏️ Кирка улучшена до <b>{user['mining_pickaxe_level']}</b> за <b>{cost:,}</b> монет! Следующее улучшение будет стоить <b>{cost2:,}</b>", parse_mode="HTML")
    else:
...
async def command_pumpkin_hunt(message: Message):
    ...
    Можно использовать раз в 30 минут.
    Награды случайны: хэллоуинские монеты, обычные монеты, или кейс.
    """
    user_id = message.from_user.id
    user = await get_user_data(user_id)
    now = datetime.datetime.now()
    ...
    if last_time:
        ...
        if last_time and now - last_time < cooldown:
            ...
            await message.answer(f"🎃 Вы недавно искали тыквы! Попробуйте снова через <b>{mins}м {secs}с</b>.", parse_mode="HTML")
            return

    # Обновляем время последнего использования
    user[cooldown_key] = now

    # Определяем результат
    outcome = random.choices(
        population=["coins", "halloween", "nothing", "halloween_case"],
    ...
    if outcome == "coins":
        ...
        await add_case_to_user(user_id, "halloween", 1)
        text += "👻 Вы нашли редкий <b>Хэллоуинский кейс!</b>"
    else:
        text += "💀 Вы бродили по кладбищу, но ничего не нашли..."
...
async def command_upgrade_rod_handler(message: Message):
    ...
    if user.get('balance_normal', 0) >= cost:
        ...
        await update_user_data(message.from_user.id, user)
        await message.answer(f"🎣 Удочка улучшена до <b>{user['fishing_rod_level']}</b> за <b>{cost:,}</b> монет! Следующее улучшение будет стоить <b>{cost2:,}</b>", parse_mode="HTML")
...
async def command_upgrade_theft_handler(message: Message):
    ...
    if user.get('balance_normal', 0) >= cost:
        ...
        await message.answer(f"🕵️ Навык кражи улучшен до <b>{user['theft_skill_level']}</b> за <b>{cost:,}</b> монет!", parse_mode="HTML")
    else: await message.answer(f"Нужно <b>{cost:,}</b>. У вас: {user.get('balance_normal', 0):,}", parse_mode="HTML")
...
async def command_upgrade_protection_handler(message: Message):
    ...
    if user.get('balance_normal', 0) >= cost:
        ...
        await message.answer(f"🛡️ Защита улучшена до <b>{user['theft_protection_level']}</b> за <b>{cost:,}</b> монет!", parse_mode="HTML")
    else: await message.answer(f"Нужно <b>{cost:,}</b>. У вас: {user.get('balance_normal', 0):,}", parse_mode="HTML")
...
async def command_invite_handler(message: Message):
    ...
    await message.answer(f"Ссылка для приглашения:\n<code>{invite_link_str}</code>\nНаграда: от <b>10,000</b> монет и больше за каждого нового пользователя!", parse_mode="HTML")
...
async def command_buy_vip_handler(message: Message):
    await message.answer_invoice(
        title="VIP-статус",
        description="Приобретение VIP-статуса",
        payload="buy_vip",
        provider_token="",
        currency="XTR",
        prices=[
            LabeledPrice(label="VIP-статус", amount=265)
        ]
    )

@dp.message(Command('buy_pro'))
@dp.message(F.text.lower() == "купить про")
async def command_buy_pro_handler(message: Message):
    await message.answer_invoice(
        title="PRO-статус",
        description="Приобретение PRO-статуса",
        payload="buy_pro",
        provider_token="",
        currency="XTR",
        prices=[
            LabeledPrice(label="PRO-статус", amount=590)
        ]
    )

@dp.message(Command('buy_special_coin'))
@dp.message(F.text.lower() == "купить особую монету")
async def command_buy_special_coin_handler(message: Message):
    await message.answer_invoice(
        title="Особая монета",
        description="Приобретение одной особой монеты",
        payload="buy_special_coin",
        provider_token="",
        currency="XTR",
        prices=[
            LabeledPrice(label="1 особая монета", amount=10)
        ]
    )

@dp.message(Command('buy_max'))
@dp.message(F.text.lower() == "купить макс")
async def command_buy_max_handler(message: Message):
    await message.answer_invoice(
        title="MAX-статус",
        description="Приобретение MAX-статуса",
        payload="buy_max",
        provider_token="",
        currency="XTR",
        prices=[
            LabeledPrice(label="MAX-статус", amount=800)
        ]
    )
'''
...
async def command_help_handler(event: Message | CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🎃Хэллоуин🎃", callback_data="halloween_commands")],
...
@dp.callback_query(F.data == "halloween_commands")
async def show_basic_commands(call: CallbackQuery):
    help_text = (
        "<b>🎃 Хэллоуинские команды:</b>\n\n"
        "<b>💰 Искать тыкву</b> - Отправиться на поиски тыкв\n"
        "<b>👹 Мое проклятие</b> - Посмотреть какое проклятие на вас наложено.\n"
        "<b>🎁 Купить хэллоуинский кейс</b> - Купить хэллоуинский кейс за 25 хэллоуинских монет.\n"
        "<b>🎁 Открыть хэллоуинсий кейс</b> - Открыть хэллоуинсий кейс.и испытать удачу\n"
        "<b>📊 Топ хэллоуин</b> - Показать топ игроков по хэллоуинским монетам."
    )
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
